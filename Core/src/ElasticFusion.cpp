...
void ElasticFusion::getModel(std::vector<Eigen::Vector4f>& xyz,
                             ...
                             std::vector<Eigen::Vector4i>& color)
{
  ...
  xyz.resize(validCount);
  normalr.resize(validCount);
  color.resize(validCount);
  int j = 0;
  for(unsigned int i = 0; i < globalModel.lastCount(); i++) {
    Eigen::Vector4f pos = mapData[(i * 3) + 0];
